...
import { GitBranch, Plus, Play, Pause, Archive, Save, ChevronDown, X } from "lucide-react";
import "./journey-builder.css";

const API = import.meta.env.VITE_API_URL ?? "";
function getToken() { return localStorage.getItem("coxa_token"); }
function getClub()  { return localStorage.getItem("coxa_selected_club_id"); }
...
// ── Node metadata ──────────────────────────────────────────────────────────
const NODE_TYPES = [
  { type: "trigger",    label: "Trigger",      icon: "⚡", color: "trigger" },
  { type: "send_email", label: "Send Email",   icon: "✉️",  color: "send_email" },
  { type: "send_push",  label: "Send Push",    icon: "🔔",  color: "send_push" },
  { type: "send_sms",   label: "Send SMS",     icon: "💬",  color: "send_sms" },
  { type: "wait",       label: "Wait",         icon: "⏱️",  color: "wait" },
  { type: "condition",  label: "Condition",    icon: "🔀",  color: "condition" },
  { type: "ab_split",   label: "A/B Split",    icon: "⚖️",  color: "ab_split" },
  { type: "end",        label: "End",          icon: "🏁",  color: "end" },
];
...
const TEMPLATES = [
  ...
  },
  {
    ...
    nodes: [
      ...
      { id: "n4", type: "condition",  label: "Renewed?",               config: { field: "membership.active", op: "eq", value: "true" }, position: { x: 100, y: 480 } },
  ...
  },
  {
    ...
    nodes: [
      ...
      { id: "n2", type: "send_push", label: "MOTM Voting Push",  config: { title: "Vote for the best player!" }, position: { x: 100, y: 200 } },
...
function NodeConfigPanel({ node, onChange }) {
  ...
  return (
    <div className="jb-config-panel">
      <h3>
        <span>{iconOf(node.type)}</span>
        {labelOf(node.type)} — Configuration
      </h3>
      <div className="jb-config-grid">
        ...
        {node.type === "send_push" && (
          <>
            ...
            <div className="jb-config-field">
              <label>Body</label>
              <input value={node.config.body ?? ""} onChange={e => set("body", e.target.value)} placeholder="Notification text" />
            </div>
          </>
        )}
          ...
          <>
            <div className="jb-config-field">
              ...
              <select value={node.config.triggerType ?? "manual"} onChange={e => set("triggerType", e.target.value)}>
                ...
                <option value="segment_exit">Exited segment</option>
              </select>
            </div>
...
function Canvas({ nodes, edges, selectedId, onSelectNode, onMoveNode, onDeleteNode, onDropFromPalette }) {
  ...
  function onMouseMove(e) {
    ...
    const x = Math.max(0, e.clientX - rect.left + scrollLeft - startX);
    const y = Math.max(0, e.clientY - rect.top  + scrollTop  - startY);
    onMoveNode(id, { x, y });
  }
  ...
  function getNodeCenter(nodeId, side) {
    ...
    const y = n.position.y + (side === "bottom" ? 70 : 0);
    return { x, y };
  }
    ...
    >
      <div className="jb-canvas-inner">
        ...
        <svg className="jb-edges-svg">
          {edges.map(edge => {
            const src = getNodeCenter(edge.source, "bottom");
            const tgt = getNodeCenter(edge.target, "top");
            ...
            return (
              <g key={edge.id}>
                ...
                />
                {edge.label && (
                  <text x={mx} y={my - 4} textAnchor="middle" fontSize="10" fill="#6b7280">{edge.label}</text>
                )}
              </g>
            );
          })}
        ...
        {nodes.length === 0 && (
          <div className="jb-empty">
            ...
            <div className="jb-empty-desc">Drag nodes from the palette or choose a template to get started</div>
          </div>
        )}
      </div>
    </div>
...
export default function AutomationWorkflowsPage() {
  ...
  const [current, setCurrent]         = useState(null);   // full journey with nodes/edges
  ...
  function addNode(type, position) {
    const id = uid();
    const newNode = { id, type, label: labelOf(type), config: {}, position };
    setCurrent(prev => ({ ...prev, nodes: [...(prev.nodes ?? []), newNode] }));
    ...
    <div className="jb-root">
      ...
      <div className="jb-sidebar">
        ...
        <div className="jb-sidebar-tabs">
          <button className={`jb-sidebar-tab${sidebarTab === "journeys" ? " active" : ""}`} onClick={() => setSidebarTab("journeys")}>
            Mine
          </button>
          <button className={`jb-sidebar-tab${sidebarTab === "templates" ? " active" : ""}`} onClick={() => setSidebarTab("templates")}>
            Templates
          </button>
        </div>
        ...
        <div className="jb-list">
          ...
          {sidebarTab === "templates" && TEMPLATES.map((tpl, i) => (
            <div key={i} className="jb-template-item" onClick={() => loadTemplate(tpl)}>
              ...
              <div className="jb-template-desc">{tpl.nodes.length} steps · {STATUS_EN[tpl.trigger?.type] ?? tpl.trigger?.type}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Modals & overlays */}
      {showCreate && (
        <CreateModal onClose={() => setShowCreate(false)} onCreate={onJourneyCreated} />
      )}
